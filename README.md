...
### 注意：本项目已内置一批默认影视源与直播源（开箱即用），仍可在管理后台自由增删/禁用。第三方源随时可能失效或更替，可用性请以实际为准。

> ⚠️ 本项目为影视信息聚合与播放工具，仅供学习与个人使用。所有影视/直播内容均来自第三方公开接口，本站不存储任何视频资源。请勿将实例公开分享或用于商业用途，并遵守所在地法律法规。
...
### 本地 Docker 构建运行（本仓库代码）

仓库内已提供 `Dockerfile`、`docker-compose.yml` 与 `docker-compose.dev.yml`。直接构建本仓库镜像并用 Redis 存储运行：
...
> ✅ **内置默认源（开箱即用）**：首次部署且后台尚未配置任何源时，系统会自动写入一批当前可用的影视源（`api_site`）与直播源（`lives`），无需手动配置即可搜索/观看。站长可在管理后台「视频源配置 / 直播源配置」中逐个禁用、删除或修改；也可在「配置文件」中粘贴自定义 JSON 后按需覆盖。

本仓库根目录提供了一份可直接套用的完整示例：**`config.example.json`**（内含 24 个影视源、默认分类与 3 组直播源），把其中的 JSON 粘贴到管理后台「配置文件」即可快速启用。
...
- `lives`：（可选）直播源配置，键为唯一标识，值为 `{ name, url, ua?, epg? }`，其中 `url` 为 m3u/m3u8 播放列表地址，`epg` 为节目单地址（可选）。
...
> ⚠️ 第三方源随时可能失效或更替。若某源搜索无结果，可在管理后台「视频源配置」中使用「检测」按钮校验，并禁用失效源。
